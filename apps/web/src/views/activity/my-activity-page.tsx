...
interface CopyFromPreviousWeekResult {
  sourceCount: number;
  createdCount: number;
  skippedCount: number;
  failedCount: number;
...
export default function MyActivityPage() {
  ...
  const weeklyCopyContext = useMemo(() => {
    const now = new Date();
    ...
    const previousWeekEntries = allActivities.filter(
      (activity) =>
        activity.activityDate >= previousWeekStart &&
        activity.activityDate <= previousWeekEnd &&
  ...
  const copyPreviousWeekMutation = useMutation({
    mutationFn: async (): Promise<CopyFromPreviousWeekResult> => {
      if (!activeTenantId) {
        return {
          sourceCount: 0,
          createdCount: 0,
      ...
      for (const existing of allActivities) {
        if (
          ...
          existing.activityDate < weeklyCopyContext.currentWeekStart ||
          existing.activityDate > weeklyCopyContext.currentWeekEnd
        ) {
          continue;
        }
        const start = parseTimeToMinutes(existing.startTime);
        const end = parseTimeToMinutes(existing.endTime);
        if (start === null || end === null || end <= start) {
          continue;
        }
        const slots = occupiedByDate.get(existing.activityDate) ?? [];
        slots.push({ start, end });
        occupiedByDate.set(existing.activityDate, slots);
      }
      ...
      for (const source of previousWeekEntries) {
        const targetDate = shiftDateKey(source.activityDate, 7);
        const start = parseTimeToMinutes(source.startTime);
        const end = parseTimeToMinutes(source.endTime);
        const taskLabel = source.taskTemplateName || "Activity";

        if (!targetDate || start === null || end === null || end <= start) {
          skippedCount += 1;
          if (details.length < 3) {
            details.push(`Skipped ${taskLabel} due to invalid date or time window.`);
          }
          continue;
        }

        const occupiedSlots = occupiedByDate.get(targetDate) ?? [];
        const isOverlapping = occupiedSlots.some((slot) => rangesOverlap(start, end, slot.start, slot.end));
        if (isOverlapping) {
          skippedCount += 1;
          if (details.length < 3) {
            details.push(`Skipped ${taskLabel} on ${targetDate} due to overlap.`);
          }
          continue;
        }

        try {
          await apiClient.post<ActivityEntry>(`/v1/tenants/${activeTenantId}/activities`, {
            body: {
              workDepartmentId: source.workDepartmentId,
          ...
          occupiedByDate.set(targetDate, occupiedSlots);
        } catch (error) {
          failedCount += 1;
          if (details.length < 3) {
            const message = error instanceof Error ? error.message : "Failed to create activity.";
            details.push(`Failed ${taskLabel} on ${targetDate}: ${message}`);
          }
        }
      }

      return {
        sourceCount: previousWeekEntries.length,
        createdCount,
        skippedCount,
        failedCount,
        details
    ...
    onSuccess: async (result) => {
      ...
      if (result.sourceCount === 0) {
        setCopyFeedback({
          tone: "warning",
          message: `No non-rejected entries found in previous week (${formatDateLabel(weeklyCopyContext.previousWeekStart)} - ${formatDateLabel(weeklyCopyContext.previousWeekEnd)}).`,
          details: []
        });
        return;
      }

      const summaryParts = [`Copied ${result.createdCount} of ${result.sourceCount} entries.`];
      if (result.skippedCount > 0) {
        summaryParts.push(`${result.skippedCount} skipped.`);
      }
      if (result.failedCount > 0) {
        summaryParts.push(`${result.failedCount} failed.`);
      }
    ...
    onError: (nextError) => {
      setCopyFeedback({
        tone: "danger",
        message: nextError instanceof Error ? nextError.message : "Failed to copy previous week entries.",
        details: []
      });
    }
  });
          ...
          <div
            className={`mt-3 rounded-md border px-3 py-2 text-sm ${
              copyFeedback.tone === "success"
                ...
                : copyFeedback.tone === "danger"
                  ? "border-red-200 bg-red-50 text-red-700"
                  : "border-amber-200 bg-amber-50 text-amber-800"
            }`}
          >
            <p>{copyFeedback.message}</p>
            {copyFeedback.details.map((detail, index) => (
              <p key={`${detail}-${index}`} className="mt-1 text-xs">
                {detail}
              </p>
            ))}
          </div>
                  ...
                  <div>
                    ...
                    <SearchableSelect
                      ...
                      onChange={(nextDepartmentId) =>
                        setFilterDraft((state) => ({ ...state, departmentId: nextDepartmentId }))
                      }
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <SearchableSelect
                    ...
                    onChange={(nextDepartmentId) => {
                      setLogDepartmentId(nextDepartmentId);
                      setLogTaskTemplateId("");
                      setLogPayload({});
                    }}
